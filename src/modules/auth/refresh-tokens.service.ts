...
export class RefreshTokensService {
  ...
  /**
   * Verifies a refresh token, marks it consumed (deleted), and issues a fresh
   * one in the same transaction. Returns the new token along with the user id
   * so the caller can mint a new access token.
   *
   ...
   */
  async rotate(token: string): Promise<{ userId: string; issued: IssuedRefreshToken }> {
    ...
    if (existing.revokedAt) {
      // A revoked token is being replayed — defense in depth: kill the whole
      // user's refresh chain. (Possible token theft scenario.)
    ...
    // Delete the old token + issue a new one atomically.
  ...
  private computeExpiry(): Date {
    const days = this.configService.getOrThrow<JwtConfig>('jwt').refreshExpiresInDays;
