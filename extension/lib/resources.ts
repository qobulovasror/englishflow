...
export async function saveWord(input: SaveWordInput): Promise<void> {
  ...
  if (input.deckId) {
    await apiFetch(`/decks/${input.deckId}/words`, {
      method: 'POST',
...
/** Aggregated stats for the popup dashboard. */
export async function getDashboard(): Promise<DashboardData> {
  const [progress, daily, recent] = await Promise.all([
    apiFetch<Progress>('/progress'),
    apiFetch<DailyWord[]>('/learning/daily'),
    apiFetch<Paginated<Word>>('/words?skip=0&take=5'),
  ]);
  return { progress, dueToday: daily.length, recent: recent.data };
}
