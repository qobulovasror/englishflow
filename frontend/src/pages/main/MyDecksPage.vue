...
function startEdit(id: string, title: string, description?: string | null, level?: CefrLevel | null) {
  editingId.value = id
...
<template>
  <div class="max-w-5xl mx-auto">
    ...
    <p class="text-gray-500 dark:text-gray-400 mb-6">Decks you created or joined. Manage your own decks here.</p>

    <AppCard v-if="showForm" class="mb-6">
      <form @submit.prevent="handleCreate" class="space-y-4">
        <AppInput v-model="createForm.title" label="Title" placeholder="e.g. Travel essentials" required />
    ...
    <div v-if="decksStore.myDecksLoading && !decksStore.myDecks.length" class="py-12 text-center text-gray-400">
      Loading…
    </div>

    <div v-else-if="!decksStore.myDecks.length" class="text-center py-12 text-gray-500 dark:text-gray-400">
    ...
    <div v-else class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
      <AppCard v-for="deck in decksStore.myDecks" :key="deck.id">
        <form v-if="editingId === deck.id" @submit.prevent="handleUpdate(deck.id)" class="space-y-3">
        ...
        <div v-else>
          ...
          <div class="flex items-center justify-between mb-3">
            ...
            <span v-if="deck.isSystem" class="text-xs px-2 py-0.5 rounded-full bg-gray-100 dark:bg-gray-700 text-gray-500">
              System
            </span>
          </div>
